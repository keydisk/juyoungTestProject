...
extension UIDevice {
    ...
    ///
    /// - Returns: 모델명
    func getDeviceModalName() -> String {
    ...
    public var uuid: String? {
        ...
        let saveUuidKey = "DeviceID"
        
        keyChain = KeychainSwift(keyPrefix: "")
        
        if let uuidData = keyChain.getData(saveUuidKey) {
            
            if let uuid = String(data: uuidData, encoding: .utf8) {
                
                return uuid
            } else if let uuid = UIDevice.current.identifierForVendor?.uuidString {
                keyChain.set(uuid, forKey: saveUuidKey)
    ...
    public var systemBigVersion: Int {
        let version = UIDevice.current.systemVersion.components(separatedBy: ".").first!
        
        return Int(version) ?? 0
    }
    
    /// 앱 버전 (CFBundleShortVersionString 값이 nil 이거나 String으로 타입캐스팅이 되지 않으면 1.0.0을 리턴)
